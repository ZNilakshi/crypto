...
import { 
  FiTrendingUp, FiUsers, FiDollarSign, FiPieChart, FiActivity 
} from "react-icons/fi";
import { FaCoins, FaMoneyCheckAlt, FaUserFriends, FaLayerGroup, FaWallet } from "react-icons/fa";
...
type CommissionRow = { level: string; layer1to3: number; layer4to6: number; };
type ProgramRow = { level: string; requirement: string; directReferrals: string; progression: string; indirect1to3: string; indirect4to6: string; };
...
export default function LayersPage() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      try {
        ...
        const [verifyRes, dashboardRes] = await Promise.all([
          ...
          axios.get(`${API_URL}/auth/dashboard`, { headers: { Authorization: `Bearer ${token}` } }),
        ]);
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-emerald-500">Loading Dashboard...</div>;
  }
  if (!user) return null;
  ...
  return (
    <div className="min-h-screen rounded-3xl text-black bg-emerald-900 p-4 relative overflow-hidden">
      <div className="max-w-6xl mx-auto space-y-8 relative z-10">
        ...
        {/* Header */}
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center space-y-4 pt-4">
        ...
        {/* Total Balance Card */}
        <motion.div className="bg-emerald-700 rounded-2xl p-6 shadow-2xl text-white relative overflow-hidden border border-emerald-500" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2, duration: 0.5 }}>
          <div className="relative z-10 text-center">
            <h2 className="text-lg font-semibold flex items-center justify-center gap-2 text-emerald-100"><FiActivity /> TOTAL COMMISSION BALANCE</h2>
            <p className="text-4xl md:text-5xl font-extrabold mt-2 text-white">
              {(balances["Layer 1-3 Commissions"] + balances["Layer 4-6 Commissions"] + balances["Leader Bonus"]).toFixed(2)} <span className="text-xl font-semibold text-emerald-100">USDT</span>
            </p>
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5 }}>
          {Object.entries(balances).map(([key, value], index) => (
            <motion.div key={key} className="bg-emerald-800 rounded-xl p-4 shadow-lg border border-emerald-600 text-white" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + index * 0.1 }}>
        ...
        {/* Tabs */}
        <motion.div className="flex flex-wrap gap-2 justify-center bg-emerald-800 p-2 rounded-xl" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.5 }}>
          {tabs.map((tab) => (
            <motion.button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`flex items-center gap-2 px-5 py-3 rounded-xl font-semibold transition-all ${activeTab === tab.key ? "bg-teal-500 text-white shadow-md" : "bg-emerald-700 text-emerald-200 hover:bg-emerald-600"}`}>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeTab} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="bg-white p-6 rounded-2xl shadow-lg border border-emerald-100">
            {activeTab === "commission" && <Table title="Commission Structure" data={commissionData} type="commission" />}
            {activeTab === "program" && <Table title="Referral Program Details" data={programDetails} type="program" />}
            {activeTab === "details" && <ReferralDetails referrals={referrals} />}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
      ...
      <div className="overflow-x-auto rounded-xl shadow-sm border border-emerald-100">
        <table className="w-full border-collapse text-emerald-800 text-sm">
          <thead className="bg-emerald-100 text-emerald-900">
            <tr>{Object.keys(data[0]).map((key, i) => <th key={i} className="border-b border-emerald-200 px-4 py-3 text-left font-semibold">{key.split(/(?=[A-Z])/).join(' ').toUpperCase()}</th>)}</tr>
          </thead>
          <tbody>
            {data.map((row, idx) => (
              <motion.tr key={idx} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.05 }} className={`border-b border-emerald-100 last:border-b-0 hover:bg-emerald-50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-emerald-50'}`}>
                {Object.values(row).map((val, i) => <td key={i} className="px-4 py-3">{typeof val === 'number' ? val.toFixed(1) : String(val)}</td>)}
              </motion.tr>
            ))}
          </tbody>
        </table>
...
function ReferralDetails({ referrals }: { referrals: Referral[] }) {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold flex items-center gap-2 mb-4"><FaUserFriends /> My Referrals</h2>
      {Object.entries(grouped).map(([layer, list]) => (
        <div key={layer} className="mb-6 overflow-x-auto border border-emerald-200 rounded-lg">
          <div className="bg-emerald-100 px-4 py-2 font-semibold gap-2 flex items-center"><FaLayerGroup /> Layer {layer} ({list.length})</div>
